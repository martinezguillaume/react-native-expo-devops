...
const owner = 'martinezguillaume';
const repo = 'react-native-expo-devops';
...
async function commentPullRequest() {
  ...
  const expo_url = `https://exp.host/@martinezguillaume/react-native-expo-devops?release-channel=${TRAVIS_PULL_REQUEST_SHA}`;
  ...
  const comment = result.data.find(comment => comment.user.login === 'martinezguillaume');
  if (comment != null) {
    // Update comment
